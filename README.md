...
Перед запуском нужно закрыть уже работающий Google Chrome, чтобы proxy-флаги применились к новому процессу.
...
Перед запуском нужно закрыть уже работающий Yandex Browser, чтобы proxy-флаги применились к новому процессу.
...
Если MATLAB уже открыт, обычный запуск будет остановлен. Для запуска еще одного экземпляра используйте:
...
В режиме `smart` домены сначала разрешаются через DNS из WireGuard-конфига. Адреса из `Peer.AllowedIPs` направляются в `work-wg`, остальные соединения идут через `direct`, как в обычном split-tunnel WireGuard.
